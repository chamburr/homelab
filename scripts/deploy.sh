...
run() {
  ...
  curl -sO https://raw.githubusercontent.com/chamburr/homelab/v2/talos/config.yaml
  curl -sO https://raw.githubusercontent.com/chamburr/homelab/v2/talos/patch.yaml
  ...
  kustomize build --enable-helm \
    "https://github.com/chamburr/homelab.git/kubernetes/argocd/argocd?ref=v2" \
    | kubectl apply -f -
  kustomize build --enable-helm \
    "https://github.com/chamburr/homelab.git/kubernetes/argocd/argocd-apps?ref=v2" \
    | kubectl apply -f -
}
